...
<body>
	...
	<h1>2. 카드 명세서</h1>
		<table class="table text-center">
			...
			<c:forEach var="bill" items="${cardBills}" varStatus="status">
				<tr>
					...
					<td><fmt:parseDate value="${bill.date}" var="date" pattern="yyyy-MM-dd" /><%-- String이였던 date를 Date객체로 바꿈 --%>
						<fmt:formatDate value="${date}" pattern="yyyy년 M월 d일" /><%-- var에 저장안하면 바로 출력됨/ date객체를 String으로 변경  --%>
					</td>
		...
		<%-- static 폴더 속 사진 추가하는 방법!! 절대경로 사용 (static위치까진 인지하고 있어, 그 후의 절대경로(/시작) --%>
