		...
		<table class="table text-center">
			...
			<c:forEach var="member" items="${member}" varStatus="status">
				<tr>
					...
					<%-- <td>${fn:substring(member.nationality, 0, 2)}-${fn:substring(member.nationality, 4, 7)}</td> --%>
					<td>${fn:replace(member.nationality, "삼국시대", "삼국 -")}</td>
					
					<%-- <td>
					<c:if test="${fn:split(member.email, '@')[0]}" />
					...
					<c:choose>
						...
						<c:out value="${fn:substring(member.introduce, 0, 15)}" /> ...
						</c:when>
						<c:otherwise>
						<c:out value="${member.introduce}"/>
						</c:otherwise>
					</c:choose>
					</td>
				</tr>
